...
// Export exports items from a tree in insertion order. It is used with NodeDB.Import().
// FIXME This should return an export iterator instead.
// FIXME This should export far less data.
func (t *ImmutableTree) Export() ([]ExportItem, error) {
	export := make([]ExportItem, 0, t.Size())

	t.root.traverse(t, false, func(node *Node) bool {
		var buf bytes.Buffer
		err := node.writeBytes(&buf)
		if err != nil {
			panic(err)
		}
		export = append(export, buf.Bytes())
		return false
	})
	return export, nil
}
...
// ExportItem is an exported key/value item. It is generated by ImmutableTree.Export(), and
// used with NodeDB.Import().
type ExportItem []byte
