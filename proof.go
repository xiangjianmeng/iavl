...
func (node *Node) pathToLeaf(t *ImmutableTree, key []byte, path *PathToLeaf) (*Node, error) {
	...
	// Note that we do not store the left child in the ProofInnerNode when we're going to add the left node
	// as part of the path, similarly we don't store the right child info when going down the right child node.
	// This is done as an optimization since the child info is going to be already stored in the next
	// ProofInnerNode in PathToLeaf.
